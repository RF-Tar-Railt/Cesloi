...
from .schedule import EventDelegate
from ..delegatesystem import SubscriberHandler
...
class TimingTask:
    ...
    def set_task(self):
        if not self.is_start:
            self.task = self.delegate.loop.create_task(self.run_task())
        else:
    ...
    def coroutine_generator(self):
        for sleep_interval in self.interval_generator():
            ...
            yield self.delegate.get_coroutine_executable_target(
                SubscriberHandler().set()(self.callable_target), {}, ""
            ), False
